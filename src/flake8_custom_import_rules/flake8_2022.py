...
class Visitor(ast.NodeVisitor):
    ...
    def visit_Compare(self, node: ast.Compare) -> None:
        if (
            ...
            and self._is_sys("version_info", node.left.value)
            and _is_index(node.left, n=0)
            and len(node.ops) == 1
            and isinstance(node.ops[0], (ast.Eq, ast.NotEq))
            and isinstance(node.comparators[0], ast.Num)
            and node.comparators[0].n == 3
        ):
            self.errors.append(
                (
                    node.left.lineno,
            ...
            self._is_sys("version", node.left)
            and len(node.ops) == 1
    ...
    def visit_Attribute(self, node: ast.Attribute) -> None:
        if isinstance(node.value, ast.Name) and node.value.id == "six" and node.attr == "PY3":
